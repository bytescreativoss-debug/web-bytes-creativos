...
const PaginaContacto = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16 sm:py-24 min-h-screen text-center">
      <h2 className="text-[#C8F000] font-black tracking-[0.4em] uppercase text-xs mb-4">Contacto</h2>
      <h3 className="text-3xl sm:text-5xl font-black italic mb-4 uppercase text-white">Hablemos <span className="text-gray-600">hoy</span></h3>
      <p className="text-gray-500 text-sm mb-12 tracking-wide">Elegí el canal que más te guste y nuestro equipo te responde a la brevedad.</p>
      <div className="flex justify-center gap-6 sm:gap-10 flex-wrap">
        <a href="https://wa.me/+5491144789797" target="_blank" rel="noopener noreferrer"
          className="bg-[#161616] border border-white/5 rounded-2xl p-8 sm:p-10 hover:border-[#C8F000] transition-all text-center flex-1 min-w-[160px] max-w-[240px] group">
          ...
          <h4 className="text-white font-bold mb-1 text-sm">WhatsApp</h4>
          <span className="text-[#C8F000] text-xs group-hover:underline">Escribinos</span>
        </a>
        <a href="mailto:bytescreativos@gmail.com"
          className="bg-[#161616] border border-white/5 rounded-2xl p-8 sm:p-10 hover:border-[#C8F000] transition-all text-center flex-1 min-w-[160px] max-w-[240px] group">
          <svg className="w-12 h-12 mx-auto mb-4 text-[#C8F000]" fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
          <h4 className="text-white font-bold mb-1 text-sm">Correo</h4>
          <span className="text-[#C8F000] text-xs group-hover:underline">bytescreativos@gmail.com</span>
        </a>
      </div>
    </div>
  );
...
export default function App() {
  ...
  function getBotResponse(text) {
    ...
    if (/hola|buenas|buen\w*|saludo|hi\b/.test(m))
      return '¡Hola! 👋 Soy el asistente de Bytes Creativos. ¿En qué puedo ayudarte?';
    if (/servicio|qué hac|que hac|ofrecen|trabajan|ayudan/.test(m))
      return 'En Bytes Creativos trabajamos en:\n• Community Management y redes sociales\n• Branding y diseño gráfico\n• Reels y videos\n• Publicidad digital (ADS)\n• Desarrollo web y tiendas online\n• Automatizaciones e IA\n• Estrategias digitales\n\n¿Querés saber más sobre alguno? 😊';
    if (/precio|costo|cuánto|cuanto|presupuest|contratar|pagar|cotiza|valor/.test(m))
      return '¡Genial! 🚀 Podés contactarnos desde la sección Contacto de nuestra web y el equipo te responderá a la brevedad.';
    if (/quién|quien|equipo|fundador|integrante|unpaz|somos/.test(m))
      return 'Somos Bytes Creativos, agencia de marketing digital y soluciones creativas. Nuestro equipo: Marilu Silva, Mariana Narbaja, Soledad Fernández y Cristian Correa 💪';
    if (/web|página|pagina|landing|tienda|ecommerce|shop|sitio/.test(m))
      return 'Hacemos desarrollo web, landing pages, websites y tiendas online 🛍️ Consultanos desde la sección Contacto para tu proyecto.';
    if (/red|social|instagram|facebook|tiktok|community|contenido|reel/.test(m))
      return 'Trabajamos gestión de redes sociales, community management, reels y producción de contenido 📱 Escribinos desde la sección Contacto.';
    if (/publicidad|anuncio|\bads\b|meta|google|campaña/.test(m))
      return 'Hacemos publicidad digital en Meta y otras plataformas 🎯 Para tu campaña, contactanos desde la sección Contacto.';
    if (/recurso|gratis|guía|guia|tutorial|librería|libreria|curso/.test(m))
      return 'Tenemos recursos gratuitos en la Librería Bytes:\n• Curso Chatbot Instagram\n• Automatizar DMs en Instagram desde Meta\n• Guía Fan Page en Facebook\n• Charla para Feriantes y Emprendedores\n• Intro a Mercado Pago\n\nEncontrás todo en la sección Recursos 📚';
    if (/bytes lab|\blab\b/.test(m))
      return 'Bytes Lab está próximamente — experiencias digitales creadas con IA 🤖 ¡Muy pronto!';
    if (/contact|hablar|reunión|reunion|entrevista|comunicar|consul/.test(m))
      return '¡Genial! 🚀 Podés contactarnos desde la sección Contacto de nuestra web y el equipo te responderá a la brevedad.';
    if (/automati|chatbot|inteligencia|\bia\b|\bbot\b/.test(m))
      return 'Hacemos automatizaciones e integraciones con IA para negocios 🤖 Contactanos desde la sección Contacto para más detalles.';
    if (/brand|logo|diseño|diseño|identidad|gráfico/.test(m))
      return 'Trabajamos branding, diseño gráfico y logos para marcas 🎨 Consultanos desde la sección Contacto.';
    return 'No tengo esa información por el momento. Te recomiendo contactarnos desde la sección Contacto de la web 😊';
  }
  ...
  const handleSendMessage = () => {
    ...
    setUserInput("");
    setTimeout(() => {
      setMessages(prev => [...prev, { role: "assistant", content: getBotResponse(userMessage.content) }]);
    }, 600);
  };
